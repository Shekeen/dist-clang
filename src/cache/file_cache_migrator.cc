...
bool Version_0_to_1(const String& cache_files_prefix, ui32 to_version,
  ...
  if (manifest.headers_size()) {
    ...
    if (manifest.v1().err()) {
      const String err_path = cache_files_prefix + ".stderr";
    ...
    if (manifest.v1().obj()) {
      const String obj_path = cache_files_prefix + ".o";
    ...
    if (manifest.v1().dep()) {
      const String dep_path = cache_files_prefix + ".d";
...
bool Version_1_to_2(const String& cache_files_prefix, ui32 to_version,
                    proto::Manifest& manifest, bool& modified) {
...
bool FileCache::Migrate(string::Hash hash, ui32 to_version) const {
  ...
  const String cache_files_prefix = GetCacheFilesPrefix(hash);
  const String manifest_path = cache_files_prefix + ".manifest";
...
#define MIGRATE(from, to)                                                 \
  if (!Version_##from##_to_##to(cache_files_prefix, to_version, manifest, \
                                modified)) {                              \
